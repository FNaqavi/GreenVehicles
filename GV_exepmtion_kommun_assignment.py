...
def GV_share_assignment():
    ...
    d['Kommun'] = [d['Kommun'][i].strip() for i in range(len(d))]       # remove white space before and after Kommun name
    d = d[d['Kommun'] != 'LIDINGÖ']
    ...
    p_lst_2012_AFV.pop(8) # AFV of Nörtalje is not available in 2012 for the passages data 
    ...
    for i in range(len(count_2012)): 
        c = count_2012[i]
        # s1 = p_lst_2012_AFV[i]
        s1 = pd.Series([p_lst_2012_AFV[i] for j in range(c)])
        s2.append(s1)  
    mask_2012['AFVt08_2012_assigned'] = pd.Series(s2)
    
    
    # if I want to assign 0 or 1 based on the probabilities
    # for i in range(len(count_2012)): 
    #     c = count_2012[i]
    #     s1 = np.random.binomial(1, p_lst_2012_AFV[i], c)
    #     s2.append(s1)  
    # mask_2012['AFVt08_2012_assigned1'] = pd.Series(s2)
    ...
    for i in range(len(count_2013)): 
        c = count_2013[i]
        # s1 = p_lst_2013_AFV[i]
        s1 = pd.Series([p_lst_2013_AFV[i] for j in range(c)])
        s2.append(s1)  
    ...
    passages1_AFV.reset_index(inplace = True)
    passages1_AFV['exempt_assigned'] = ser
    passages1_AFV.index = passages1_AFV['index']
    
    # # assigned (0: after 2009, 1: before 2008)
    p_assigned = pd.concat([passages1_AFV,passages1_CV]).reset_index(drop = True)
    p_assigned.drop(columns=['index'],inplace = True)
